// Nitro server route: GET /api/pitches?lat=&lng=&radius=
// OSM-площадки + кэш в supabase. См. server/api/geocode.ts для контекста.
...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    const lng = parseFloat(typeof query.lng === "string" ? query.lng : "");
    const radius = Math.min(
      40000,
      Math.max(500, parseInt(typeof query.radius === "string" ? query.radius : "15000", 10) || 15000),
    );
    ...
    return { items: data ?? [], cached: !stale };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    console.error("[pitches] unhandled", msg);
