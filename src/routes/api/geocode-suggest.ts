 ...
 * Возвращает до 5 вариантов от Я.Геокодера для автодополнения адреса.
 *
 * TanStack Start 1.131: createServerFileRoute + .methods (ServerRoute).
 * Экспорт ServerRoute — плагин помещает маршрут в serverRouteTree,
 * createStartHandler ловит его ДО executeRouter() → возвращает чистый JSON.
 */
...
export const ServerRoute = createServerFileRoute("/api/geocode-suggest")
  .methods({
    GET: async ({ request }) => {
      try {
