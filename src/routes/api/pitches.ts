 ...
 * OSM-площадки + кэш в supabase (public_pitches).
 *
...
export const ServerRoute = createServerFileRoute("/api/pitches")
  .methods({
    GET: async ({ request }) => {
      try {
        ...
        const radius = Math.min(
          40000,
          Math.max(500, parseInt(u.searchParams.get("radius") ?? "15000", 10) || 15000),
        );
