...
### Серии, чаты менеджера, защита оплат (09–10.06.2026)
- ✅ Модель ёмкости поля (миграция 20260609130000): full=6/два_трети=4/треть=2 юнита, лимит 6. get_free_slots + book_venue считают конфликты МЕЖДУ размерами.
- ✅ Слот-пикер в /create: занятое время недоступно, realtime-обновление сетки.
- ✅ Запрет удаления оплаченных игр (триггер) + manager_cancel_game (архив + возврат-нотификации) + «Связь с менеджером» в /my.
- ✅ DM менеджер↔организатор без дружбы (has_manager_relation, can_dm_with) + mark-read входящих.
- ✅ Серии игр #32 (миграция 20260609140000): /create тоггл повтора → request_series; /my «Мои серии»; /manager аппрув с пропуском занятых дат; у менеджера повтор external-броней.
- ✅ /manager вкладка «Чаты» (manager_list_dialogs).
- ✅ Хедер: ссылки «Админка»/«Менеджер» по ролям.

### Автозакрытие игр без счёта (11.06.2026)
- ✅ pg_cron job `process-unscored-games` (раз в час, миграция 20260611100000): через 24ч без счёта — нотификация+пуш оргу и капитанам; через 3 дня — game_results 0:0 (notes 'auto…'), архив, нотификация участникам.
- ✅ /my показывает только неархивные игры (архив = история профиля).
- ⚠️ Секреты пушей на VPS живут в `/var/www/af-sport/secrets.env` (вне git!) — deploy.sh подгружает его перед билдом, потому что git stash затирает правки коммитнутого .env.

### РКН-пакет (10.06.2026)
- ✅ /auth: два раздельных согласия при регистрации (обработка + распространение), факт пишется в user_metadata (pd_consent, pd_public_consent, pd_consented_at).
- ✅ /privacy и /personal-data: актуальный состав ПД (email, push, координаты, Метрика), инфраструктура = self-hosted на VPS reg.ru в РФ, раздел о публичных данных и распространении.
- ✅ Шрифты локально через @fontsource (Google Fonts убран), og-превью → /og-image.png (нужно положить файл в public/), Метрика webvisor:false, email везде hello@af-sport.ru.
...
| #28b | Админка менеджера: Финансы/Безопасность/Настройки | ✅ ЧАСТИЧНО 11.06.2026: вкладка «Финансы» (read-only) — выручка за 6 мес., бар-чарт, разбивка по площадкам/источникам, realtime (`manager.finance.tsx`, без миграций). Осталось: вывод средств (нужен эквайринг + финмодель), Безопасность (PIN/смена пароля), Настройки (реквизиты). |
...
| #34 | РКН: уведомление об обработке ПД | Приложение подготовлено (см. §9 от 10.06.2026): согласия в /auth, локальные шрифты, без r2.dev, Метрика без webvisor, email hello@af-sport.ru. Осталось руками: 1) создать ящик hello@af-sport.ru; 2) положить public/og-image.png; 3) подать уведомление (ОГРН из ЕГРЮЛ, адрес ЦОД из договора reg.ru, трансграничка «не осуществляется»). |
| #31 | ~~Фильтр свободных стадионов~~ | ✅ СДЕЛАНО 10.06.2026: RPC `find_available_stadiums(date, time?, duration)` (миграция 20260610150000), фильтр-бар на /stadiums (дата+время+длительность, validateSearch), быстрый поиск на главной → /stadiums?date=&time=&dur=. Фильтр применим только к партнёрским стадионам; на /games не делали — там уже есть свои фильтры времени. |
...
**Закрытие приватных медиа в nginx (#33, разово):**
```nginx
# В /etc/nginx/sites-available/af-sport ПЕРЕД блоком location /uploads/:
location ~ ^/uploads/(chat-images|dm-media)/ { return 404; }
```
После правки: `nginx -t && systemctl reload nginx`. Файлы этих бакетов раздаёт
только Node через /api/media по подписанной ссылке. В /privacy убрать фразу о
доступности медиа по прямой ссылке после включения (§6.1).
...
7. ~~`supabase-pooler` (Supavisor)~~ — ✅ выключен и закомментирован в docker-compose.yml 10.06.2026 (не нужен, ходим через Kong). Если когда-то понадобится пулер — раскомментировать блок и проверить что `VAULT_ENC_KEY` строго 32 hex.
...
10. ~~`/sb/` блок в nginx~~ — ✅ удалён 10.06.2026 (бэкап: `/etc/nginx/sites-available/af-sport.bak-sb`).
